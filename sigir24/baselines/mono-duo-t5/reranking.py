...
def parse_args():
    ...
    parser.add_argument('--input-dataset', type=str, help='The input dataset, e.g., reneuir-2024/dl-top-10-docs-20240701-training.', required=True)
...
def rerank(model, tok_model, top_k, input_dataset, output_directory):
    monot5 = tira.pt.from_submission('ir-benchmarks/tira-ir-starter/MonoT5 3b (tira-ir-starter-gygaggle)', input_dataset)
    pt_dataset = pt.get_dataset(f'irds:{input_dataset}')
    df = monot5(pt_dataset.get_topics('title'))
    docsstore = pt_dataset.irds_ref().docs_store()
    df = df[df['rank'] <= top_k]
    df['text'] = df['docno'].apply(lambda i: docsstore.get(i).default_text())
    print(df.head(10))
...
if __name__ == '__main__':
    ...
    rerank(args.model, args.tokenizer, args.top_k, args.input_dataset, args.output)
